...
void ArbitraryIndexingTest()
{ 
  //fseek ( stdout , -10 , SEEK_CUR );
  //const char* buff = "Somebuffer";
  //fprintf(stdout, buff);
  //fputs(buff);
  //int len = strlen(buff);
  //memcpy(reinterpret_cast<wchar_t *>(stdout), buff, len);
  //for(int i = 0; i < 2000; ++i)
  //std::cout << reinterpret_cast<char *>(stdout)[i];
  //std::cout << std::endl;
  //stdout[0]
  //std::cout << "SDF" << std::endl;
  //std::cout << '[' << reinterpret_cast<char *>(stdout) << ']';
  //std::cout << stdout << '\n';
}
...
//char *buff = positionBuff;
  ...
  {  
    ...
    for(int i = 0; i < height - 2; ++i)
    {
      ...
      {
        if((i + j) % 2 == 0)
          oh += Coloring::COLORCODE_RED;
        else 
          oh += Coloring::COLORCODE_WHITE;
        oh += 'A';
        //if(j == width-1)
